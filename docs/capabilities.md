...
| 86 | Issue/工单集成 | ✅ | **v0.5.21** lib/tracker.ts 单一实现三端消费 —— GitHub REST v3 真集成（鉴权解析 ORG_GH_TOKEN env > config.gh_token > GH_TOKEN；GHE 端点 ORG_GH_API；token 脱敏 maskToken；repo 形状校验防注入；15s 超时保护；HTTP 错误可诊断传播；无 token 诚实拒绝带三条指引）。issueList/Get/Create/Comment/SetState 五动作。三端：CLI `org issue list/get/create/comment/close/reopen` · 工具环 `issue_list/issue_get`（只读）+ `issue_create/issue_comment/issue_close`（file_write 门 + 审批在环 —— 远端工单与本地文件同级治理）· Web 📋 面板；tests/tracker 18 例（含工具环 e2e：scripted 剧本 → mock 网关真实 fetch → 数据回灌；异步 spawn 驱动 —— spawnSync 阻塞事件循环会让 mock 网关挂死）。真实车道实测：org issue list myh2026/org 5 条渲染 + 本批 issue 由 org issue create 自举提交 |
...
| 107 | 堆栈跟踪分析 | ✅ | **v0.5.23** lib/stacktrace.ts `analyzeStackTrace`：粘贴崩溃输出 → ①四语言帧解析（TS/JS `at fn (file:line:col)` 三形态含 async · PY `File "…", line N, in fn` · Rust `panicked at` 主位帧 + backtrace `at` 行 · HSL 泛形 `at file.hsl:l:c`）②帧富化：外部分类（node_modules/node:internal/site-packages/.cargo 等 21 特征 → external，保留上下文不入 appFrames）· 工作区文件存在性（exists）· 源码行原文（snippet，越界不读盘——jail 铁律）· 包围符号（同文件最近函数类作用域 fn/graph/class/impl —— const 局部变量不误报）③根因提示库 18 基因四生态（JS null-deref/call-non-fn/undefined-ref · fs ENOENT/EACCES · net · SyntaxError · PY Key/Index/Import/NoneType/Recursion · Rust unwrap 反引号形态/index/overflow · HSL S-19/S 族）：每条 cause + 3 步行动清单 + severity 分级 + 关联帧（帧 raw 命中 → 回落最内层用户帧）④帽纪律：帧 60 截断 + 输入 64KB + 重复帧去重。三端：CLI `org debug stack --text/文件/--json/--self-test` · 工具环 `stack_analyze`（text/file 双形态，file 过 pathjail，只读）· Web 🧵 面板（POST 粘贴 + GET 文件 + 自检，/api/govex/debug action=stack）；tests/stacktrace 28 例。诚实边界：高频崩溃族启发式模式库（无数据流分析）· 符号化是词法级 |
...
**统计（v0.5.23 堆栈自动分析后口径，tests/check.test.ts 防漂移守卫锁定）**：主 Agent 150 项 → ✅ 127 · 🟡 23 · ⬜ 0；专家 25 项 → ✅ 25 · 🟡 0 · ⬜ 0。（v0.5.20.2：#88 🟡→✅ collab+RBAC 合围实态；vendored 0.2.66→0.2.68。v0.5.21：#86+#82 双 🟡→✅ GitHub REST 真集成 + B-23/24/25 三 bugfix + vendored 0.2.69/0.2.70。v0.5.22：#146 SAST + #65 依赖管理 + #104 选择性重跑 三 🟡→✅ —— SAST 多引擎降级链 ruff→bandit→内置规则永远有产出 · 七工具探测+白名单安装车道 · flaky 台账+三选择器。v0.5.23：#107 堆栈自动分析 🟡→✅ —— 四语言帧解析 + 符号化 + 外部分类 + 18 基因根因提示库三端落地。）
